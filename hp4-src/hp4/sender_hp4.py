...
class INTHead(Packet):
    name = "INT"
    fields_desc = [
        IntField("position", 0),
        LongField("timestamp1", 0),
        LongField("timestamp2", 0),
        LongField("timestamp3", 0)
    ]
...
def main():
    ...
    while(1):
        ...
        #p = INT() / SrcRoute(num_valid = len(port_list)) / port_str / msg
        '''pkt = Ether(dst="00:00:00:00:00:01") / msg
        dstlen = len(pkt.dst)
        srclen = len(pkt.src)
        totallen = len(pkt)
        print pkt.show()
        print dstlen, srclen, totallen'''
        '''pkt = Ether(dst="00:00:00:00:00:00:00:01") / msg
        print pkt.show()
        sendp(pkt, iface = "eth0")

        pkt = Ether(dst="00:00:00:00:00:00:00:02") / msg
        print pkt.show()
        sendp(pkt, iface = "eth0")

        pkt = Ether(dst="00:00:00:00:00:00:00:03") / msg
        print pkt.show()'''
        #sendp(pkt, iface = "eth0")

        pkt = INTHead() / Ether(dst="00:04:00:00:00:01") / msg
        pkt.show()
        sendp(pkt, iface = "eth0")

        #pkt = EthPacket(dst=3) / msg
        #sendp(pkt, iface = "eth0")
        # print msg
